...
public class StatisticsService {
    ...
    public Map<String, Integer> getMostPopularBooks(int limit) {
        ...
        // Đếm số lần mỗi sách được mượn
        for (Loan loan : loans) {
            for (var detail : loan.getLoanDetails()) {
                String bookTitle = detail.getBook() != null ? detail.getBook().getTitle() : detail.getISBN();

                popularBooks.put(bookTitle, popularBooks.getOrDefault(bookTitle, 0) + 1);
            }
        }

        // Sắp xếp và lấy top N sách
        return popularBooks;
    }
    ...
    public Map<String, Integer> getMostActiveReaders(int limit) {
        ...
        for (Loan loan : loans) {
            ...
            activeReaders.put(readerName, activeReaders.getOrDefault(readerName, 0) + bookCount);
        }
